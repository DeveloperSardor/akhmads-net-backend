...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await distributionService.deliverAd(botId, SendToChatId.toString(), SendToChatId);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const ad = await adCreationService.createAdWithValidation(req.userId, req.body);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      // Check ownership
      if (ad.advertiserId !== req.userId && !['ADMIN', 'SUPER_ADMIN', 'MODERATOR'].includes(req.userRole)) {
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const performance = await advertiserAnalytics.getAdPerformance(req.params.id, req.userId);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const buffer = await advertiserAnalytics.exportImpressions(req.params.id, req.userId);

      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.setHeader('Content-Disposition', `attachment; filename=impressions-${req.params.id}.xlsx`);
      res.send(buffer);
    } catch (error) {
      next(error);
    }
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const ad = await adSchedulingService.removeSchedule(req.params.id, req.userId);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const stats = await dailyStatsService.getAdDailyStats(req.params.id, days);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const stats = await dailyStatsService.getHourlyDistribution(req.params.id);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const days = parseInt(req.query.days || 30);
      const stats = await dailyStatsService.getAdvertiserDailyStats(req.userId, days);
